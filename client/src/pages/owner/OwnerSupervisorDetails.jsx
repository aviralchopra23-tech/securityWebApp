...
import { assignSupervisor, getLocations, getSupervisorById, updateUser } from "../../api/ownerApi";
import "../../styles/ownerSupervisorDetails.css";

export default function OwnerSupervisorDetails() {
  ...
  const fillForm = (s) => {
    setForm({
      ...
      dateJoined: s.dateJoined ? String(s.dateJoined).slice(0, 10) : "",
      payRate: s.payRate ?? "",
      password: "",
      assignedLocationId: s.assignedLocationId?._id || "",
    });
  };
  ...
  const handleSave = async () => {
    ...
    if (form.assignedLocationId) {
      payload.assignedLocationId = form.assignedLocationId;
    }
    ...
    try {
      setIsSaving(true);
      await updateUser(supervisorId, payload);
      if (form.assignedLocationId && form.assignedLocationId !== data.assignedLocationId?._id) {
        await assignSupervisor({
          supervisorId,
          locationId: form.assignedLocationId,
        });
      }
      ...
      setWarning(err.response?.data?.message || "Failed to update supervisor.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (error || !data) return <p>{error || "Supervisor not found."}</p>;

  return (
    <div className="supervisor-details-page">
      <div className="details-header-row">
        <button type="button" className="btn-back" onClick={() => navigate("/owner/users/supervisors")}>Back</button>
        {!isEditing ? (
      ...
      <div className="details-card">
        ...
        <div className="details-grid">
          ...
          <div className="field"><span>Location</span><strong>{data.assignedLocationId?.name || "Not assigned"}</strong></div>
      ...
      {isEditing && (
        <div className="edit-card">
          <div className="edit-grid">
            ...
            <label><span>Assigned Location</span>
