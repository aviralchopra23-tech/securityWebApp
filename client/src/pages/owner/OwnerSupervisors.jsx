...
export default function OwnerSupervisors() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [supData, locData] = await Promise.all([
        getSupervisors(),
        getLocations()
      ]);
  ...
  return (
    <div className="supervisors-page">
      ...
      <div className="form-row">
        <input placeholder="First Name" value={newSupervisor.firstName}
          onChange={(e) => setNewSupervisor({ ...newSupervisor, firstName: e.target.value })} />
        <input placeholder="Last Name" value={newSupervisor.lastName}
          onChange={(e) => setNewSupervisor({ ...newSupervisor, lastName: e.target.value })} />
        <input placeholder="Email" value={newSupervisor.email}
          onChange={(e) => setNewSupervisor({ ...newSupervisor, email: e.target.value })} />
        <input type="password" placeholder="Password" value={newSupervisor.password}
          onChange={(e) => setNewSupervisor({ ...newSupervisor, password: e.target.value })} />
        <button className="primary-btn" onClick={handleCreateSupervisor}>Create</button>
      </div>
      ...
      <div className="form-row">
        ...
        <button className="primary-btn" onClick={handleAssignSupervisor}>Assign</button>
      </div>
          ...
          <li key={s._id} className="supervisor-item">
            {editingId === s._id ? (
              <div className="edit-row">
                ...
                <button className="btn-save" onClick={() => saveEdit(s._id)}>Save</button>
